...
function Baseult:MenuValueDefault()
  self.menu = "Baseult"
  self.menu_keybin_auto = self:MenuBool("Auto", true)
  self.menu_keybin_combo = self:MenuKeyBinding("Do Not Use Ultimate in Fight", 32)
end

function Baseult:OnDrawMenu()
  if Menu_Begin(self.menu) then
    self.menu_keybin_auto = Menu_Bool("Auto", self.menu_keybin_auto, self.menu)
    self.menu_keybin_combo = Menu_KeyBinding("Do Not Use Ultimate in Fight", self.menu_keybin_combo, self.menu)
    Menu_End()
  end
end

function Baseult:MenuBool(stringKey, bool)
  return ReadIniBoolean(self.menu, stringKey, bool)
end

function Baseult:MenuSliderInt(stringKey, valueDefault)
  return ReadIniInteger(self.menu, stringKey, valueDefault)
end

function Baseult:MenuSliderFloat(stringKey, valueDefault)
  return ReadIniFloat(self.menu, stringKey, valueDefault)
end

function Baseult:MenuComboBox(stringKey, valueDefault)
  return ReadIniInteger(self.menu, stringKey, valueDefault)
end

function Baseult:MenuKeyBinding(stringKey, valueDefault)
  return ReadIniInteger(self.menu, stringKey, valueDefault)
end
  ...
  if CanCast(_R) and not self.Rcast then
    for i, recall in pairs(self.Recalling) do
      ...
      if dmg >= recall.hp and self.enemySpawnPos ~= nil then
        ...
        if TimeToRecall < TimeToHit and TimeToHit < 7.8 and TimeToHit - TimeToRecall < 1.5 and dmg >= recall.hp and self.menu_keybin_auto and GetKeyPress(self.menu_keybin_combo) == 0 then
